...
public_users.post("/login",async (req,res) => {
  ...
  return res.status(200).json({message : username+ " logged in successfully" ,accessToken});
  } else {
...
public_users.get('/',async function (req, res) {
  return await res.status(200).send(JSON.stringify(books));
});
...
public_users.get('/isbn/:isbn',async function (req, res) {
  ...
  return await res.status(200).send(JSON.stringify(book));
 });
...
public_users.get('/author/:author',async function (req, res) {
  ...
  return await res.status(200).send(JSON.stringify(bookByAuthor));
});
...
public_users.get('/review/:isbn',async function (req, res) {
  ...
  if(!book)
    return res.status(404).json({error : 'there is no book with this number '+isbn}) 
  return await res.status(200).send(JSON.stringify(book.reviews));
});
